...
class DBHelper(context: Context) :
    ...
    companion object {
        const val DATABASE_NAME = "SanPhamDB"
        const val DATABASE_VERSION =3
        const val TABLE_NAME = "Sanphams"
...
//    }
    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL("CREATE TABLE Sanphams (id INTEGER PRIMARY KEY, masp TEXT, 'ten' TEXT, 'soluong' INTEGER)")
...
//        db.execSQL("INSERT INTO Products (name, price, imageResId) VALUES ('Sản phẩm B', 9000, ${R.drawable.saigonxanh})")
    ...
    fun insertSanPham(masp: String, tensp: String, soluong: Int): Boolean {
        val db = writableDatabase
        val values = ContentValues()
        values.put("masp", masp)
        values.put("ten", tensp)
        values.put("soluong", soluong)
        val result = db.insert("Sanphams", null, values)
        return result != -1L
    }
    fun updateSanPham(masp: String, tensp: String, soluong: Int): Boolean {
        val db = writableDatabase
        val values = ContentValues()
        values.put("ten", tensp)
        values.put("soluong", soluong)
        val result = db.update("Sanphams", values, "masp=?", arrayOf(masp))
        return result > 0
    }

    fun deleteSanPham(masp: String): Boolean {
        val db = writableDatabase
        val result = db.delete("Sanphams", "masp=?", arrayOf(masp))
        return result > 0
    }
//    fun getAllTask(): List<SanPham> {
//        val db = readableDatabase
//        val list = mutableListOf<SanPham>()
//        val cursor = db.rawQuery(
//            "SELECT * FROM $TABLE_NAME",
