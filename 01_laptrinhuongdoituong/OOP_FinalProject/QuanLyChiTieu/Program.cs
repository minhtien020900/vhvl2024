    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (!isLogin)
            {
                ...
                {
                    isLogin = true;
                    userID = Login(email, pass).NguoidungId;
                    tenNguoiDung = Login(email, pass).Ten;
                }
                else
                {
                    isLogin = false;
                ...
                if (isLogin)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        Thread.Sleep(300);  // Tạm dừng 300ms giữa mỗi dấu chấm
                    }
                }
            }
            ...
            while (isRunning)
            {
                ...
                Console.WriteLine("1. Thống kê tổng tiền theo danh mục.");
                Console.WriteLine("2. Hiển thị danh sách các thu chi theo khoảng thời gian.");
                Console.WriteLine("3. Phân tích thu/chi theo khoảng thời gian.");
                Console.WriteLine("4. Tính chênh lệch của thu và chi theo khoảng thời gian.");
                Console.WriteLine("5. Tổng hợp chi tiêu theo khoảng thời gian.");
                Console.WriteLine("6. So sánh chi tiêu của 2 tháng.");
                Console.WriteLine("7. Top 5 chi tiền nhiều nhất theo khoảng thời gian.");
                Console.WriteLine("8. Danh sách các danh mục thu/chi của người dùng.");
                Console.WriteLine("9. Tra cứu các thu chi theo khoảng tiền.");
                Console.WriteLine("10. Thống kê tỷ lệ thu chi của các danh mục theo khoảng thời gian.");
                ...
                switch (choice)
                {
                    ...
                    case "3":
                        Console.Clear();
                        PhanTichThuChiTheoKhoangTg();
                        Console.WriteLine("\nNhấn bất kỳ phím nào để quay lại menu.");
                        Console.ReadKey();
                        break;
                    case "4":
                        Console.Clear();
                        TinhChenhLechThuChiTheoKhoangTg();
                        Console.WriteLine("\nNhấn bất kỳ phím nào để quay lại menu.");
                        Console.ReadKey();
                        break;
                    case "5":
        ...
        static public void TongTienTheoDanhMucNguoiDung()
        {
            ...
            {
                Console.WriteLine($"{item.Ten} - {item.TenDanhMuc} - {item.Loai} - {item.TongSoTien:N0} VND");
            }
        }
        static public void DanhSachThuChiTheoThangCuaNguoiDung()
        {
            ...
            Console.WriteLine($"\n--- Danh sách các thu chi của người dùng từ ngày {startDate.ToShortDateString()} đến {endDate.ToShortDateString()} ---\n");

            foreach (var item in listThu)

            {
                Console.WriteLine($"# Ngày {item.NgayThang.ToString().Split(' ')[0]} - {item.DanhMuc.TenDanhMuc} - {item.SoTien:N0} VND");
            }
        }
        static public void PhanTichThuChiTheoKhoangTg()
        {
            Console.Write("Nhập ngày bắt đầu (theo định dạng MM/dd/yyyy ví dụ: 12/25/2024): ");
            string date1 = Console.ReadLine();
            DateTime startDate = DateTime.Parse(date1);
            ...
            var danhSachThuchiTheoDm = _db.LichSuThuChiLs
                ...
                .Select(lsb => new
                {
                    ...
                    tongtientrungbinh = lsb.Average(x => x.SoTien),

                }).ToList();

            Console.WriteLine($"\n--- Phân tích thu/chi từ ngày {startDate.ToShortDateString()} đến {endDate.ToShortDateString()} ---\n");
            foreach (var item in danhSachThuchiTheoDm)
            {
                Console.WriteLine($"# Loại: {item.loai} | Danh mục: {item.Dm} | Tổng số tiền: {item.tongtien:N0} VND | Số lượt giao dịch: {item.solan} | Số tiền trung bình: {item.tongtientrungbinh:N0} VND");

                //Console.WriteLine($"# Loại: {item.loai} - Danh mục: {item.Dm} - Tổng số tiền: {item.tongtien} - Số lượt giao dịch: {item.solan} - Số tiền trung bình mỗi lượt: {item.tongtientrungbinh}");
            }
        }
        static public void TinhChenhLechThuChiTheoKhoangTg()
        {
            Console.Write("Nhập ngày bắt đầu (theo định dạng MM/dd/yyyy ví dụ: 12/25/2024): ");
            string date1 = Console.ReadLine();
            DateTime startDate = DateTime.Parse(date1);
            ...
            Console.WriteLine($"\n--- Chênh lệnh giữa thu/chi từ ngày {startDate.ToShortDateString()} đến {endDate.ToShortDateString()} ---\n");
            foreach (var item in danhSachThuChiTheoLoai)
            {
                Console.WriteLine($"#Tổng {item.loai}: {item.tongtien:n0} VND");
            }
            ...
            Console.WriteLine($"\nChêch lệch giữa thu và chi: {tienthu - tienchi:n0} VND");
        }
        static public void ChiCuaNguoiDungTheoNgayThang()
        {
            ...
            var thuChiTheoNgay = _db.LichSuThuChiLs.Where(x => x.NguoiDungId == userID && x.NgayThang >= startDate && x.NgayThang <= endDate && x.DanhMuc.loaiThuChi.Loai == "Chi")
                ...
                .Select(group => new
                {
                    ...
                    items = group.Select(g =>
                   new
                   {
                       loai = g.DanhMuc.loaiThuChi.Loai,
                       ngayThang = g.NgayThang,
                       soTien = g.SoTien
                   }),
            ...
            Console.WriteLine($"\n--- Danh sách chi tiêu trong khoảng thời gian từ {startDate.ToShortDateString()} đến {endDate.ToShortDateString()} ---\n");
            ...
            foreach (var item in thuChiTheoNgay)
            {
                ...
                {
                    Console.WriteLine($"# Loại: {subItem.loai} - Danh Mục: {item.key} - Ngày tháng: {subItem.ngayThang.ToString().Split(' ')[0]} - Số tiền: {subItem.soTien:n0} VND");
                }


                Console.WriteLine($"\nTổng chi tiêu cho danh mục '{item.key}': {item.tongTien:n0} VND\n");
            }
            //Console.WriteLine($"Tổng tiền tiêu: {tongtien:n0} VND");

        }
        static void sosanhchitheothang()
        {
            Console.Write("Nhập tháng thứ nhất (số nguyên 1-12): ");
            int thang1 = int.Parse(Console.ReadLine());
            Console.Write("Nhập năm thứ nhất: ");
            int nam1 = int.Parse(Console.ReadLine());

            Console.Write("Nhập tháng thứ hai (số nguyên 1-12): ");
            int thang2 = int.Parse(Console.ReadLine());
            Console.Write("Nhập năm thứ hai: ");
            ...
            // Hiển thị kết quả
            Console.WriteLine($"\nTổng chi tháng {thang1}/{nam1}: {tongChiThang1:n0} VND");
            Console.WriteLine($"Tổng chi tháng {thang2}/{nam2}: {tongChiThang2:n0} VND");

            // So sánh và kết luận
            if (tongChiThang1 > tongChiThang2)
            {
                var chechlech = tongChiThang1 - tongChiThang2;
                Console.WriteLine($"Tháng {thang1}/{nam1} có chi tiêu nhiều hơn tháng {thang2}/{nam2} là {chechlech:n0} VND.");
            }
            else if (tongChiThang1 < tongChiThang2)
            {
                var chechlech = tongChiThang2 - tongChiThang1;
                Console.WriteLine($"Tháng {thang1}/{nam1} có chi tiêu ít hơn tháng {thang2}/{nam2}  là {chechlech:n0} VND.");
            }
            else
            {
        ...
        {
            Console.Write("Nhập tháng cần xem (1-12): ");
            int thang = int.Parse(Console.ReadLine());
            Console.Write("Nhập năm cần xem: ");
            ...
            if (topChiTieu.Any())
            {
                Console.WriteLine($"\nTop 5 khoản chi lớn nhất trong tháng {thang}/{nam}:\n");
        ...
        static public void TimKiemCacGiaoDich()
        {
            Console.Write("Nhập số tiền nhỏ nhất: ");
            double sotien1 = double.Parse(Console.ReadLine());
            Console.Write("Nhập số tiền lớn nhất: ");
            double sotien2 = double.Parse(Console.ReadLine());

            var danhsach = _db.LichSuThuChiLs.Where(x => x.NguoiDungId == userID && x.SoTien >= sotien1 && x.SoTien <= sotien2)
                .GroupBy(x => x.DanhMuc.loaiThuChi.Loai)
                .Select(group => new
                {
                    key = group.Key,
                    subItems = group.Select(g =>
                      new
                      {
                          loai = g.DanhMuc.loaiThuChi.Loai,
                          ngayThang = g.NgayThang,
                ...
                .ToList();
            Console.WriteLine($"\nDanh sách chi tiêu có số tiền trong khoảng từ {sotien1:n0} VND đến {sotien2:n0} VND");
            foreach (var item in danhsach)
            {
                ...
                {
                    Console.WriteLine($"# Ngày: {subItem.ngayThang.ToString().Split(' ')[0]} - Danh mục: {subItem.danhMuc} - Số tiền:{subItem.tien:n0} VND");
                }
