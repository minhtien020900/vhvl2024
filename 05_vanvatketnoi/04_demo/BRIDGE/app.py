...
play_thread_stop = threading.Event()  # dừng hẳn thread
current_mode = "UART"  # mặc định
# <div class="d-flex justify-content-between mb-3">
#   <button class="btn btn-warning text-white" onclick="setMode('UART')">Mode UART</button>
#   <button class="btn btn-dark text-white" onclick="setMode('BUTTON')">Mode BUTTON</button>
# </div>
...
audio.addEventListener('pause', () => { fetch('/pause', {method:'POST'}); });
audio.addEventListener('play', () => { fetch('/resume', {method:'POST'}); });

window.addEventListener('beforeunload', () => { fetch('/stop_thread', {method:'POST'}); });

function sendStart() { fetch('/start', {method:'POST'}); }
function sendStop() { fetch('/stop', {method:'POST'}); }

function setMode(mode) { fetch('/set_mode/'+mode, {method:'POST'}).then(res=>res.json()).then(d=>alert(d.status)); }
...
function uploadFile() {
    ...
    fetch('/upload', {method:'POST', body:formData}).then(res=>res.json()).then(d=>alert(JSON.stringify(d)))
}
...
def set_mode(mode):
    global current_mode
    mode = mode.upper()
    if mode not in ["UART","BUTTON"]:
        return jsonify({"status":"Mode không hợp lệ"}),400
    current_mode = mode
    ser.write(f"MODE|{mode}\r\n".encode())
    return jsonify({"status":f"Mode đã set: {mode}"})
